...
% Remove ACM-specific reference block (non-archival submission)
...
\usepackage{amsmath}
\let\Bbbk\relax % avoid clash between LaTeX kernel and amssymb redefinition
\usepackage{amssymb}
...
\author{Bryan Cheng}
\authornote{All three authors contributed equally to this work.}
...
\renewcommand{\shortauthors}{Cheng, Jin, and Zhang}
...
Multi-task learning research has produced strikingly inconsistent results: sometimes joint training improves performance substantially, sometimes it actively harms accuracy through negative transfer \cite{fifty2021efficiently, wang2019negative}. We show this inconsistency stems from violating a fundamental requirement: \textbf{gradient-based task similarity analysis requires tasks to share training instances}. Standard benchmarks like MoleculeNet \cite{wu2018moleculenet} operate with $<$5\% instance overlap between tasks, placing them in a regime where gradient analysis is provably unreliable. This provides the first principled explanation for seven years of inconsistent MTL results.

\textbf{Why existing approaches fall short.} Gradient-based methods like PCGrad \cite{yu2020gradient} and GradNorm \cite{chen2018gradnorm} treat gradient conflicts as problems to resolve rather than signals to interpret. Task similarity metrics \cite{zamir2018taskonomy, fifty2021efficiently, standley2020tasks} require training multiple models---prohibitively expensive for hundreds of tasks. The field lacks a principled framework for predicting task relationships \textit{before} expensive multi-task training.
...
\end{equation}%
The interpretation: $G_{ij} > 0$ indicates synergy (shared mechanisms); $G_{ij} < 0$ indicates conflict (competing demands); $G_{ij} \approx 0$ indicates independence. Prior work treats conflicts as problems to \textit{resolve} \cite{yu2020gradient, chen2018gradnorm}; we propose they are signals to \textit{interpret}.
...
\begin{itemize}[nosep,topsep=0pt]
    \item \textbf{MoleculeNet} \cite{wu2018moleculenet}: $<$5\% overlap (ESOL, Lipo, BACE, BBBP from different sources)
    \item \textbf{TDC} \cite{huang2021therapeutics}: 8--14\% overlap across ADMET domains
    \item \textbf{ChEMBL} \cite{gaulton2012chembl}: Assays run on disjoint compound libraries
\end{itemize}%
...
\textbf{Gradient extraction.} We compute per-task losses and extract gradients with respect to shared encoder parameters, enabling multiple task gradients from a single forward pass. We compute $\G$ every 10 steps and average over the final 20\% of training.

\textbf{Validation.} Our core metric is $r(\G, \E)$---the correlation between gradient conflicts and empirical property correlations computed from held-out data. We also test biological validity via hierarchical clustering on $\G$ compared to pathway annotations using Adjusted Rand Index \cite{hubert1985ari}.

\textbf{Architecture.} We use a Graph Convolutional Network \cite{kipf2017semi} with task-specific MLP heads; results are robust across architectures (\S\ref{sec:robustness}).
...
Gradient patterns stabilize early: by epoch 20, correlation with the final matrix reaches $r = 0.73$. Comparing architectures (ECFP \cite{rogers2010ecfp}, GCN, GAT \cite{velickovic2018gat}, 1D-CNN \cite{weininger1988smiles}), learned representations produce consistent patterns ($r = 0.71$--$0.81$), while ECFP differs substantially ($r = 0.38$--$0.46$).
...
\textbf{Gradient-based MTL optimization.} MGDA \cite{sener2018multi}, GradNorm \cite{chen2018gradnorm}, PCGrad \cite{yu2020gradient}, CAGrad \cite{liu2021conflict}, Nash-MTL \cite{navon2022multi}, and uncertainty weighting \cite{kendall2018multi} treat gradient conflicts as optimization challenges to \textit{resolve} rather than signals to \textit{interpret}. Recent surveys \cite{zhang2021survey} comprehensively cover these methods but none examines data conditions under which conflicts become meaningful.

\textbf{Task relationship discovery.} Taskonomy \cite{zamir2018taskonomy} and Task2Vec \cite{achille2019task2vec} discover relationships via transfer learning or Fisher embeddings, requiring multiple models. On molecular tasks, gradient similarity outperforms Task2Vec ($r = 0.65$ vs $r \approx 0$). \citet{fifty2021efficiently} and \citet{standley2020tasks} study task groupings but rely on expensive combinatorial search; none articulates sample overlap as a requirement.

\textbf{MTL in molecular property prediction.} MoleculeNet \cite{wu2018moleculenet} noted ``merging uncorrelated tasks has only moderate effect'' without measuring cross-task overlap. GNN pretraining \cite{hu2020pretraining} and task-specific architectures \cite{gilmer2017neural, yang2019analyzing} improve molecular representations, but task selection for MTL remains a hyperparameter.
...
Code is available at \url{https://github.com/JasperZG/gradientmtl}.
...
Algorithm~\ref{alg:gradient} describes the gradient conflict matrix computation. The key insight is retaining the computation graph to extract multiple task gradients from a single forward pass.
...
\FOR{$k = 1$ to $K$}
    ...
    \STATE $\mathbf{g}_k \leftarrow \nabla_\theta \mathcal{L}_k$ \COMMENT{retain\_graph}
\ENDFOR
\FOR{$i = 1$ to $K$}
    \FOR{$j = 1$ to $K$}
...
\begin{enumerate}[nosep,topsep=2pt]
    \item \textbf{ECFP+MLP}: ECFP4 fingerprints \cite{rogers2010ecfp} (radius=2, 2048 bits) with 3-layer MLP encoder [2048$\rightarrow$512$\rightarrow$256]
    \item \textbf{GCN}: Graph Convolutional Network \cite{kipf2017semi} as described above
    \item \textbf{GAT}: Graph Attention Network \cite{velickovic2018gat} with 4 attention heads per layer
    \item \textbf{1D-CNN}: Character-level CNN on SMILES strings \cite{weininger1988smiles} (embedding dim=64, kernel sizes [3,5,7])
\end{enumerate}
...
\begin{table}[H]
\caption{Complete overlap threshold characterization.}
\begin{center}
\begin{small}
\begin{tabular}{ccccc}
\toprule
Overlap & $r(\G, \E)$ & $\rho(\G, \E)$ & $p$-value & $n$ \\
\midrule
...
{\small Sigmoid fit: $L = 0.82$, $k = 0.15$, $x_0 = 29.7$, $R^2 = 0.94$.}
...
\textbf{Artificial vs.\ natural low-overlap.} Our controlled experiment uses random partitioning to reduce overlap, which tests whether overlap is a \textit{necessary} condition for gradient interpretability. However, real low-overlap scenarios arise when different tasks are measured on genuinely different chemical spaces. The kinase results ($r=0.67$ at $\sim$20\% average overlap) illustrate this complexity.

\subsection{Gradient Dynamics Full Results}
\begin{table}[H]
\caption{Gradient matrix correlations across epochs.}
\begin{center}
\begin{small}
\begin{tabular}{lcccccc}
\toprule
 & Ep1 & Ep5 & Ep10 & Ep20 & Ep50 & Final \\
\midrule
Ep1 & 1.00 & .78 & .80 & .71 & .65 & .63 \\
Ep5 & --- & 1.00 & .87 & .84 & .70 & .68 \\
Ep10 & --- & --- & 1.00 & .82 & .73 & .71 \\
Ep20 & --- & --- & --- & 1.00 & .75 & .73 \\
Ep50 & --- & --- & --- & --- & 1.00 & .89 \\
\bottomrule
...
We compare gradient similarity against Task2Vec \cite{achille2019task2vec}, which computes task embeddings via Fisher information. On Tox21 (12 tasks, 66 pairs):
